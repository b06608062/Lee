// mark
// 2671
// 值域離散化 + Lazy Segment Tree + 掃描線 + 二分搜 + 積分面積差值
// 矩形面積並集
class SegmentTree {
public:
  struct Node {
...
public:
  double separateSquares(vector<vector<int>> &squares) {
    ...
    for (auto &sq : squares) {
      ...
      xs.push_back(lx);
      xs.push_back(rx);
      evs.emplace_back(y, lx, rx, 1);
      evs.emplace_back(y + side, lx, rx, -1);
    }
    ...
    int idx = lo - 1;
    auto [area_at_start, current_sum_len] = records[idx];
    double remaining_area_needed = (res / 2.0) - area_at_start;
    return evs[idx].y + remaining_area_needed / current_sum_len;
  }
};